...
def training_loop(
    ...
    D = dnnlib.util.construct_class_by_name(**D_kwargs, **common_kwargs).train().requires_grad_(False).to(
        device)  # subclass of torch.nn.Module
    DS = dnnlib.util.construct_class_by_name(**DS_kwargs, **common_kwargs).train().requires_grad_(False).to(
        device)  # subclass of torch.nn.Module
    G_ema = copy.deepcopy(G).eval()  # deepcopy can make sure they are correct.
    #Sid - to add DS if there is a pretrained Model
    ...
    loss = dnnlib.util.construct_class_by_name(
        device=device, G=G, D=D, DS=DS, **loss_kwargs)  # subclass of training.loss.Loss
    
    phases = []

    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval),
                                                   ('D', D, D_opt_kwargs, D_reg_interval), ('DS', DS, DS_opt_kwargs, DS_reg_interval)]:
